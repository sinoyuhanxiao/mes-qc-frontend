<template>
  <div style="overflow-y: hidden">
    ...
    <!-- Order Details with el-descriptions -->
    <el-descriptions
        :column="2"
        border
        title="工单详情"
      ...
      <el-descriptions-item label="任务备注" v-if="currentOrder.description" :span="2">
        {{ currentOrder.description }}
      </el-descriptions-item>
      <el-descriptions-item label="状态">
        <el-tag :type="getQcOrderStateTagData(currentOrder.state).type" size="small">
          {{ getQcOrderStateTagData(currentOrder.state).label }}
        </el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="创建时间" v-if="currentOrder.created_at">
        {{ formatDate(currentOrder.created_at) }}
      </el-descriptions-item>
      <el-descriptions-item label="创建者" v-if="currentOrder.created_by">
        <UserReference :userId="currentOrder.created_by" />
      </el-descriptions-item>
      <el-descriptions-item label="更新时间" v-if="currentOrder.updated_at">
        {{ formatDate(currentOrder.updated_at) }}
      </el-descriptions-item>
      <el-descriptions-item label="更新者" v-if="currentOrder.updated_by">
    ...
    <div v-if="dispatches.length > 0">
      <el-card
        ...
        <!-- Buttons(Collapsed, Pause, Resume) -->
        <div style="display: flex;
          ...
          <el-button v-if="dispatch.state === 1" type="info" @click="handlePauseOrderDispatch(dispatch.id)">
            暂停
          </el-button>
          <el-button v-if="dispatch.state === 5" type="info" @click="handleResumeOrderDispatch(dispatch.id)">
            重启
          </el-button>
        </div>
        ...
        <div v-show="!dispatch.collapsed">
          <el-descriptions
              ...
              border
              title="派发计划详情">
            <el-descriptions-item label="名称" v-if="dispatch.name">
              {{ dispatch.name }}
            </el-descriptions-item>
            <el-descriptions-item label="派发计划ID">
              {{ dispatch.id }}
            </el-descriptions-item>
            <el-descriptions-item label="备注" v-if="dispatch.description" :span="2">
              {{ dispatch.description }}
            </el-descriptions-item>
            <el-descriptions-item label="状态">
              <el-tag :type="getDispatchStateTagData(dispatch.state).type" size="small">
            ...
            <el-descriptions-item label="执行计划" v-if="dispatch.cron_expression">
              {{ humanizeCronInChinese(unnormalizeCronExpression(dispatch.cron_expression)) }}
            </el-descriptions-item>
            <el-descriptions-item label="派发次数上限" v-if="dispatch.type === 'regular'">
              {{ dispatch.dispatch_limit === -1 ? "无限制" : dispatch.dispatch_limit }}
            </el-descriptions-item>
            <el-descriptions-item label="周期开始时间" v-if="dispatch.start_time">
              {{ formatDate(dispatch.start_time) }}
            </el-descriptions-item>
            <el-descriptions-item label="周期停止时间" v-if="dispatch.end_time">
              {{ formatDate(dispatch.end_time) }}
            </el-descriptions-item>
            <el-descriptions-item label="执行时间" v-if="dispatch.custom_time">
              {{ formatDate(dispatch.custom_time) }}
            </el-descriptions-item>
            <el-descriptions-item label="下次派发时间" v-if="dispatch.nextExecutionTime && dispatch.type === 'regular'">
            ...
            <el-descriptions-item label="创建者" v-if="dispatch.created_by">
              <UserReference :userId="dispatch.created_by" />
            </el-descriptions-item>
            <el-descriptions-item label="创建时间" v-if="dispatch.created_at">
              {{ formatDate(dispatch.created_at) }}
            </el-descriptions-item>
            <el-descriptions-item label="更新者" v-if="dispatch.updated_by">
              <UserReference :userId="dispatch.updated_by"/>
            </el-descriptions-item>
            <el-descriptions-item label="更新时间" v-if="dispatch.updated_at">
        ...
        <div v-show="!dispatch.collapsed"
          ...
          <!-- System Associations / Scheduling -->
          <el-divider></el-divider>
          <el-descriptions
              :column="2"
              border
              title="质检任务配置">
            <el-descriptions-item label="派发任务时限 (分钟)">
          ...
          <el-descriptions
                ...
                <el-tag
                  ...
                  <el-popover effect="light" trigger="hover" placement="top" width="300px">
                    <template #default>
                      ...
                      <div v-if="product.image_paths" style="margin-top: 10px;">
                        <el-image
            ...
            <el-descriptions-item label="原料列表">
              <div class="tags">
                <el-tag
            ...
            <el-descriptions-item label="生产工单列表">
              <div class="tags">
                <el-tag
            ...
            <el-descriptions-item label="维护工单列表">
              <div class="tags">
                <el-tag
...
export default {
  ...
  methods: {
    ...
    async fetchNextExecutionTime(dispatch) {
      ...
      try {
        ...
        if (response.data.data) {
          dispatch.nextExecutionTime = new Date(response.data.data).getTime();
        }
      } catch (error) {
    ...
    toggleCollapse(index) {
      this.dispatches[index].collapsed = !this.dispatches[index].collapsed;
    },
